...
def g_bin(y):
    return 1 - (y.count(0) / len(y))**2 - (y.count(1) / len(y))**2


def g_gain(y, left, right):
    return g_bin(y) - (len(left) / len(y)) * g_bin(left) - (len(right) / len(y)) * g_bin(right)
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for split in possible_splits:
            ...
            if len(left) == 0 or len(right) == 0:
                continue
            gini_gain = g_gain(y.tolist(), left, right)
